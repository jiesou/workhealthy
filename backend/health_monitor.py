...
class HealthMonitor:
    ...
    def _monitor_loop(self):
        ...
        try:
            # 获取当前状态
            detection_status = self.video_processor.status
            
            # 获取数据库会话
            db = next(get_db())
            
            # 1. 检查工作状态
            self._check_working_status(db)
            
            # 2. 检查活动状态
            current_time = datetime.now()
            if (not self.last_activity_check or 
                (current_time - self.last_activity_check).total_seconds() >= self.activity_check_interval):
                self._check_activity_status(db, detection_status)
            ...
            if (not self.last_water_check or 
                ...
                self._check_water_intake(db, detection_status)
    ...
    def _check_working_status(self, db: Session):
        """
        检查工作状态
        ...
        elif not person_detected and self.current_working_session_id:
            crud.end_working_session(db, self.current_working_session_id)
            self.current_working_session_id = None
    
    def _check_activity_status(self, db: Session, status: VideoProcessor.DetectionStatus):
        """
        检查活动状态
    ...
    def _check_water_intake(self, db: Session, status: VideoProcessor.DetectionStatus):
        """
        检查喝水状态
        ...
        参数:
            ...
            status: 当前状态
        """
        cup_detected = status.is_cup_detected
    ...
    def output_insights(self):
        """获取当前状态见解 解胖服务端策略：字符串在服务端合成"""
        status = self.video_processor.status
        insights = {}
        ...
        try:
            ...
            insights["today_work_duration_message"] += str(timedelta(seconds=today_work_duration))
    ...
    def refresh_generator_summary_health(self):
        """刷新生成器摘要"""
        self.generator_service.refresh_summary_health(self.video_processor.status)
